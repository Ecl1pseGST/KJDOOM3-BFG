...
void idWeapon::UpdateFlashPosition()
{
	...
	if( isPlayerFlashlight )
	{
		...
		// KJ: Full rework - the flashlight now shines from the currently
		// equipped weapon instead of this dedicated (invisible) flashlight
		// entity's own joint. Two wins from this:
		//
		// 1) It actually fixes centering, for good this time. This entity's
		// own joint was positioned via a hand-tuned, largely guesswork
		// transform (base offset/rotation + oscillating sway) meant to
		// approximate "somewhere near the player's hands," with no real
		// relationship to the crosshair - which is what caused three rounds
		// of drift/parallax bugs. The currently-held weapon's own muzzle/
		// barrel joint is already positioned correctly every frame by the
		// normal weapon-rendering pipeline (the same joint actual gunfire
		// uses), so reusing it via GetMuzzlePositionWithHacks() - the same
		// utility idWeapon already relies on for reliable muzzle-based
		// effects, including its handling of weapons with badly-oriented or
		// missing barrel joints - gives an origin that's already right,
		// instead of a second hand-tuned approximation to maintain.
		//
		// 2) It's the foundation for weapon-specific flashlight attachments
		// later: the light is now anchored to whatever weapon is actually
		// out, so a future attachment system just needs to toggle this per
		// weapon rather than reconciling two independently positioned
		// viewmodels.
		idWeapon* activeWeapon = ( owner != NULL ) ? owner->weapon.GetEntity() : NULL;
		if( activeWeapon != NULL && activeWeapon != this )
		{
			idMat3 unusedAxis;
			activeWeapon->GetMuzzlePositionWithHacks( muzzleFlash.origin, unusedAxis );
		}
		// else: no distinct weapon equipped right now (fists, the flashlight
		// itself in classic mode, or mid-switch) - fall back to this
		// entity's own joint transform, already set above.

		muzzleFlash.origin += owner->GetViewBob();

		// Aim along the player's true view axis, exactly like
		// GetProjectileLaunchOriginAndAxis() already does for actual gunfire
		// elsewhere in this file - plain camera forward, no convergence
		// calculation needed. A weapon-barrel origin sits close enough to
		// the true eye line that parallel aim reads as correct in practice,
		// the same way it already does for bullets - unlike the old
		// shoulder-ish origin, which was offset far enough to need the
		// convergence math the previous version of this fix added.
		//
		// This still can't be a plain `axis = playerViewAxis` assignment:
		// this light is a projected spotlight (flashPointLight "0" in
		// weapon_flashlight*.def) whose fixed LOCAL target/up/right vectors
		// (flashTarget "0 0 1280", flashUp "0 640 0", flashRight "-640 0 0")
		// reach along LOCAL Z, not local X - a different convention from
		// idTech4's own world axis (axis[0]=forward, axis[2]=up). The basis
		// below is built directly in that Z-forward/Y-up/X-right convention,
		// permuting playerViewAxis's own (already-orthonormal) rows into it.
		idMat3 newAxis;
		newAxis[2] = playerViewAxis[0];		// forward -> local Z (flashTarget's axis)
		newAxis[1] = playerViewAxis[2];		// up -> local Y (flashUp's axis)
		newAxis[0] = -playerViewAxis[1];		// right -> local X (flashRight's axis; idTech4 axis[1] is "left")
		muzzleFlash.axis = newAxis;

		idAngles adjustAng = idAngles( fraccos * yscale, 0.0f, fraccos2 * pscale );
		idAngles bobAngles = owner->GetViewBobAngles();
		SwapValues( bobAngles.pitch, bobAngles.roll );
		adjustAng += bobAngles * 3.0f;
		muzzleFlash.axis = adjustAng.ToMat3() * muzzleFlash.axis;
	}
