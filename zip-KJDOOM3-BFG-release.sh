...
filename="KJDOOM-3-BFG-${buildversion}-lite-win64-${date_string}-git-${git_hash}.7z"

# Copy the base file to the new filename
cp "KJDOOM-3-BFG-1.6.0.x-lite-win64-yyyymmdd-git-xxxxxxx.7z" "$filename"

# Copy necessary files
cp build/Release/KJDoom3BFG.exe .
cp build/tools/compilers/release/kjdmap.exe .

# Add files to the archive
7z a "$filename" KJDoom3BFG.exe kjdmap.exe
...
7z a "$filename" base/_kjdoom_blood.pk4
7z a "$filename" base/_kjdoom_core.pk4
...
fullname="KJDOOM-3-BFG-${buildversion}-full-win64-${date_string}-git-${git_hash}.7z"
cp "$filename" "$fullname"

7z a "$fullname" base/_kjdoom_global_illumination_data.pk4
